...
\caption{Predictions at six query points. The heteroscedastic model
  recovers the input-dependent $\sigma$ within 10--16\% across the
  range. The MSE model has no $\sigma$ at all.}
\end{table}

The heteroscedastic model tracks the true $\sigma$ within 10--16\%
across the range (worst case at $x = 5.5$: predicted $1.625$ versus
true $1.933$). The MSE model has no $\sigma$ at all; it only predicts
the mean.
...
\caption{Test Gaussian NLL on 200 held-out samples (lower is better).
  Both values omit the $\tfrac{1}{2}\log(2\pi)$ constant, which cancels
  in the comparison; they are valid for relative ranking but should not
  be compared to benchmarks that include the constant.
