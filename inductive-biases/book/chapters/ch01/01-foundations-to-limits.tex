% DEFINED:   sec:nn-as-function, sec:linear-limits, eq:residual, fig:xor-boundary
...
diagonal corners of the unit square. No line separates them, as the left
panel of \Cref{fig:xor-boundary} shows.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/ch01-xor-boundary}
  \caption{XOR decision boundaries on the unit square (blue points are
  class~0, red are class~1). \emph{Left}: a single linear unit (logistic
  regression) draws only one straight boundary, collapses to
  $p \approx 0.5$ everywhere, and cannot separate the diagonal classes.
  \emph{Right}: a 2-8-1 \texttt{Tanh} MLP (\Cref{sec:mlp-backprop}) bends
  the boundary and separates them. Same output head, different
  architecture.}
