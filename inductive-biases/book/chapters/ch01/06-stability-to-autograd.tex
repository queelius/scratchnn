...
% RESOLVED:  sec:logits-logprobs, eq:logodds
% FORWARD:   (none)
...
\Cref{sec:logits-logprobs}: since only logit differences carry
information (the log-odds of \Cref{eq:logodds}), a shared shift of the
logits changes nothing, and we spend that freedom to keep the
exponentials in a safe range. Stability here is not a new idea bolted on. It is the gauge
...
it on four configurations (seed~42):
